...
class PartaiAdapter : BaseRecyclerAdapter() {
    var selectedItem: PoliticalParties? = null
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        (holder as PartaiViewHolder).bind(data[position] as PoliticalParties)
    }

    inner class PartaiViewHolder(override val containerView: View) : RecyclerView.ViewHolder(containerView), LayoutContainer {
        fun bind(politicalParties: PoliticalParties) {
            if (politicalParties.image?.url != null) {
                ...
                partai_pilihan_foto_partai.loadUrl(politicalParties.image?.url, R.drawable.ic_avatar_placeholder)
                partai_pilihan_nama.text = politicalParties.name
                partai_pilihan_no_urut.text = "No Urut ${politicalParties.number}"
            } else {
            ...
            if (politicalParties.id.equals(selectedItem?.id)) {
            ...
            itemView.setOnClickListener {
                setSelectedData(politicalParties)
            }
        }
    }

    fun setSelectedData(politicalParties: PoliticalParties) {
        selectedItem = politicalParties
        println("${selectedItem?.id} : ${politicalParties.id}")
        selectedItem?.let { listener.onSelectItem(it) }
        notifyDataSetChanged()
    }

    interface Listener {
        fun onSelectItem(politicalParties: PoliticalParties)
    }
}